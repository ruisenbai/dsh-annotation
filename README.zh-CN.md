...
- 将两行注解记录分成“待发送”“已排队”“已发送”三组，并使用 DSH 官方图标和 Tooltip 提供定位、编辑、补充、删除、导出和清空操作。
...
- 点击定位后，将数字编号所在的正文行垂直居中到真实会话滚动区域。
...
打开 DSH Web 页面，在一条已完成回复中选中文字，点击“添加注解”，在紧凑输入框中填写意见并点击对号创建草稿。草稿会出现在输入框上方；检查分组列表、按需填写整体要求，然后发送。
