...
describe('CSS highlight manager', () => {
  it('aggregates message ranges, activates one range, and disposes names', () => {
    ...
    expect((values.get('dsh-annotation') as FakeHighlight).ranges).toEqual([first, second])
    manager.activate('message-1', first)
    expect((values.get('dsh-annotation-active') as FakeHighlight).ranges).toEqual([first])
    manager.remove('message-1')
    expect((values.get('dsh-annotation') as FakeHighlight).ranges).toEqual([second])
    manager.dispose()
