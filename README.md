...
![DSH 主设置面板中的注解配置](docs/assets/inline-comments-settings.png)
...
- 模型回复逐条对照：Host 提示词要求模型按注解顺序逐条回答、每段以“注解 N：”开头、不合并注解，并在每段前输出隐藏的 `dsh-annotation-reply` 关联标记、结尾输出 `dsh-annotation` acknowledgement 标记。Client 保留原有“注解 N”文字，不复制或覆盖，只在能够可靠定位的文字边界内添加透明交互层；标记间的多余空行和同一回复内多批重复编号不会使后续标签错位，同一标记区间内最先匹配的完整标签有重复时则保留普通原文。悬停 300ms 或键盘聚焦显示编号、原文和注解摘要，不阻挡拖选与复制；激活标签会定位原文。
...
- 支持逐条删除草稿并撤销最近一次删除。注解汇总框底部不提供本地存储占用、导出下载或批量清空控件；本地自动保存和刷新恢复仍然可用。
- 独立的 **设置 → 注解** 页面使用 DSH 官方 `Switch` 和 `Tag` 组件，并通过 dsh-market 公开更新 API 检查当前版本、安装更新、显示进度、按失败类型允许强制更新，以及按能力提供回滚、刷新和 Host 重启操作。
...
- 编号只使用选区结束行现有的安全留白，不预留正文 gutter、不改 padding；同一视觉行的多条注解合并为一个“×N”标记，成员按编号升序显示，没有安全空间或无法恢复选区时仍可从汇总条查看。高亮保持弱化，仅活动注解加强提示，普通正文悬停或点击不弹批注；思考过程、视口、字体和缩放变化合并触发布局测量。
- 点击编号打开查看卡；查看卡与正文编辑器依据实测视口、滚动容器和官方输入框位置布局，优先使用正文侧边现有空白，其次下方、上方，窄屏或空间不足时使用安全范围内的可滚动面板，跟随滚动并支持 125%/200% CSS zoom。汇总框发起的编辑仍原位显示，编辑已有草稿保留可撤销删除；未保存内容的外部点击保护与输入法选词保护同样适用。
- 定位原文（包括激活模型回复标签）时，仅在原文确实位于折叠的 Turn 容器内才先展开，再把完整引用垂直居中到真实会话或窗口滚动区域，并校正 CSS 缩放；引用范围显示一次平滑淡出的临时背景，后一次定位会撤销前一次临时效果，不会清除标记或编辑器拥有的持久高亮。
...
- **草稿：**仅在浏览器中，可编辑。
- **已排队：**已观测到 DSH Inbox 的 `next-turn` 项，尚未写入模型历史。
...
- 一次选区必须位于同一条助手回复内，跨消息选区会被拒绝。Markdown 文件链接文字可参与选区与恢复，但识别依赖当前宿主的 DOM；升级时需重新验证，详见[兼容性说明](docs/compatibility.md#high-risk-integration-points)。
