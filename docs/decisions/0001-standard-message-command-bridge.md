...
The claim deliberately omits `CommandClaim.images` because mixed image and annotation input is outside this plugin's submission format. DSH therefore refuses a mixed draft before invoking the submit callback. On submission, the Client freezes the current draft set, uses the visible composer text as `overallRequirement`, and calls the existing internal command. A failed transaction retains the claim, official draft, immutable payload, and submission id for retry.
