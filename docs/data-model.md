...
An outbox entry contains the immutable payload, target Session id, deterministic message id, attempt count, optional attachment metadata, and status. The `attachments` metadata stores `{ count, kinds, mediaTypes, names }`: `kinds` retains the ordered `image`/`file` tags, while `mediaTypes` and `names` describe images only. It contains no attachment bytes or temporary file-upload receipts. Legacy `images: { count, mediaTypes, names }` metadata remains readable and represents an image-only batch; when both fields exist, `attachments` owns the retry requirement. These optional fields keep `storageVersion: 2` and the current storage key.

An invalid optional `editorDraft` is omitted so valid drafts and immutable retry records can still recover. Invalid core arrays, attachment metadata, provenance, or unsupported versions are not partially trusted; the Client starts with an empty state and shows a storage warning.
...
A client-side item-count or size rejection occurs before any Host call and leaves new annotations as drafts. A recorded attachment batch requires the same count and ordered kinds before any Host call; missing attachments or different kinds reject the retry. Legacy image-only metadata requires that count of images, and a record without attachment metadata cannot gain attachments on retry. Adding or removing attachments requires discarding the old record and creating a new submission. The guard does not compare file bytes, so the user must reselect the original attachments after a refresh. A transport failure is ambiguous and therefore remains retryable with the same id: the official composer draft and armed annotation claim stay in place. Loading a persisted `sending` or still-unobserved `accepted` entry converts it to `failed`, preserving the frozen payload after a refresh or tab crash and making the same id retryable. Host deduplication resolves a retry that arrived after an unseen successful admission.
